  ...
  return `<div class="shell">
    <header class="topbar">
      ...
      <nav class="nav">
        ...
        <a class="btn secondary hide-mobile" href="/faq">FAQ</a>
    ...
    ${content}
    <footer class="footer">© 2026 QR AJN • <a href="/faq" data-nav>FAQ</a> • <a href="/help" data-nav>Help</a> • <a href="/privacy.html">Privacy</a> • <a href="/delete-account.html">Delete account</a> • qrajn.online</footer>
  </div>`;
}

function setQrAjnMetaByName(name, content) {
...
function setQrAjnMetaByProperty(property, content) {
...
function setQrAjnCanonical(url) {
...
function setQrAjnSeo({title, description, canonical, robots = 'index,follow,max-image-preview:large'}) {
  document.title = title;
  setQrAjnMetaByName('description', description);
  setQrAjnMetaByName('robots', robots);
  setQrAjnCanonical(canonical);
  setQrAjnMetaByProperty('og:site_name', 'QR AJN');
  setQrAjnMetaByProperty('og:type', 'website');
  setQrAjnMetaByProperty('og:title', title);
  setQrAjnMetaByProperty('og:description', description);
  setQrAjnMetaByProperty('og:url', canonical);
  setQrAjnMetaByProperty('og:image', `${publicDomain}/app_icon_192.png`);
  setQrAjnMetaByName('twitter:card', 'summary');
  setQrAjnMetaByName('twitter:title', title);
  setQrAjnMetaByName('twitter:description', description);
  setQrAjnMetaByName('twitter:image', `${publicDomain}/app_icon_192.png`);
}

function applyQrAjnHomeSeo() {
  setQrAjnSeo({
    title: 'QR AJN â€” Digital Business Profiles, Business QR & Dynamic QR',
    description: 'QR AJN helps businesses create digital business profiles, branded QR identities and editable dynamic QR destinations on qrajn.online.',
    canonical: `${publicDomain}/`,
  });
}

function applyQrAjnPublicProfileSeo(profile, requestedSlug) {
  const slug = normalizeSlug(profile.slug || requestedSlug);
  const name = String(profile.name || 'QR AJN Business').trim();
  const details = [profile.title, profile.company, profile.address]
    .filter(Boolean)
    .map((item) => String(item).trim())
    .filter(Boolean)
    .join(' â€¢ ');
  const description = details
    ? `${name} â€” ${details}. View contact and business information on QR AJN.`
    : `${name} digital business profile on QR AJN. View published contact and business information.`;

  setQrAjnSeo({
    title: `${name} | QR AJN Business Profile`,
    description: description.slice(0, 160),
    canonical: `${publicDomain}/b/${encodeURIComponent(slug)}`,
  });
}
...
async function route() {
  ...
  try {
    if (path.startsWith('/b/')) return renderPublicProfile(path.slice(3));
    ...
    if (path.startsWith('/q/')) return location.replace(`/r/${encodeURIComponent(path.slice(3))}`);
    ...
    if (path === '/pricing') return renderSimplePage('Pricing', 'QR AJN 7.1 focuses on stable core workflows. Ads and paid plans remain disabled until production acceptance testing is complete.');
    if (path === '/help') return renderSimplePage('Help Center', 'Create a profile, publish it, share the public URL or QR, and update the profile later without reprinting.');
    if (path === '/faq') return renderFaq();
    if (path === '/contact') return renderSimplePage('Contact', 'Use the contact information published by the QR AJN developer or the support channel configured for your deployment.');
    if (path === '/blog') return renderSimplePage('QR AJN Updates', 'Product updates, QR safety guidance and digital business identity guides will appear here.');
...
function renderLanding() {
  ...
  root.innerHTML = shell(`
    <section class="hero">
      <div>
        <span class="status">QR AJN V7.1 • Production closure</span>
    ...
    <section class="section" id="features">
      ...
      <div class="grid">
        ...
        ${feature('📇', 'Business-first identity', 'Professional cards, dynamic links, public profiles and sharing without forcing customers to install an app.')}
      </div>
    </section>
    <section class="section">
      <div class="panel callout">
        <div><h2>Free QR tools. Business account only when you need cloud features.</h2><p>Scanning and static QR creation remain usable without compulsory login. Sign in only when you choose cloud profiles, dynamic links, leads or analytics.</p></div>
...
async function requireAccount() {
  if (!configured) {
    root.innerHTML = shell(`<div class="error panel"><h1>Connect Firebase first</h1><p>Run <code>RUN_QR_AJN_V7_1_PRODUCTION_CLOSURE.ps1</code> with your new Firebase project ID.</p></div>`);
    bindNavigation();
    return false;
  }
  if (!currentUser) {
    ...
    <div class="builder-shell">
      ...
      <form class="panel builder-form" id="profileForm">
        <section class="builder-page active" data-page="0">
          ...
          ${field('slug', 'Profile URL', p.slug, 'text', 'qrajn.online/b/your-name')}
        ...
        <section class="builder-page" data-page="2">
          ...
          ${textarea('galleryUrls', 'Gallery image URLs', lines(p.galleryUrls), 4)}\n          <div class="field upload-row"><input id="galleryFile" type="file" accept="image/jpeg,image/png,image/webp,image/gif"><button type="button" class="btn secondary" id="galleryUploadBtn">Upload gallery image</button></div>
...
function bindBuilder(initial) {
  ...
  document.getElementById('galleryUploadBtn')?.addEventListener('click', uploadGalleryImage);
  ...
  form.onsubmit = async (event) => {
    ...
    try {
      ...
      const oldSlug = normalizeSlug(initial?.slug || '');
      if (oldSlug && oldSlug !== p.slug) {
        const oldRef = doc(db, 'business_profiles', oldSlug);
        const oldSnapshot = await getDoc(oldRef);
        if (oldSnapshot.exists() && oldSnapshot.data().ownerUid === currentUser.uid) {
          await deleteDoc(oldRef);
        }
      }
      initial.slug = p.slug;
      message.innerHTML = `Saved. <a href="/b/${encodeURIComponent(p.slug)}" data-nav>Open public profile</a>`;
      bindNavigation();
    } catch (error) {
...
async function uploadBusinessMedia(fieldId) {
  ...
  const kindMap = {photoUrl: 'profilePhoto', logoUrl: 'logo', coverUrl: 'cover', brochureUrl: 'brochure'};
  const mediaKind = kindMap[fieldId];
  if (!mediaKind) {
    alert('This upload field is not enabled for production storage.');
    return;
  }
  const isBrochure = mediaKind === 'brochure';
  const allowedImageTypes = new Set(['image/jpeg', 'image/png', 'image/webp', 'image/gif']);
  if ((isBrochure && file.type !== 'application/pdf') || (!isBrochure && !allowedImageTypes.has(file.type))) {
    alert(isBrochure ? 'Only PDF brochures are supported.' : 'Use JPG, PNG, WebP or GIF images.');
    return;
  }
  const max = isBrochure ? 10 * 1024 * 1024 : 5 * 1024 * 1024;
  if (file.size > max) {
    alert(`File is too large. Maximum ${isBrochure ? '10 MB' : '5 MB'}.`);
    return;
  }
  const safeName = `${Date.now()}_${file.name.replace(/[^A-Za-z0-9._-]/g, '_')}`;
  const objectRef = ref(storage, `business_media/${currentUser.uid}/${mediaKind}/${safeName}`);
...
async function uploadGalleryImage() {
  ...
  if (!allowed.has(file.type)) return alert('Use JPG, PNG, WebP or GIF images.');
  if (file.size > 5 * 1024 * 1024) return alert('Gallery image must be smaller than 5 MB.');
  const safeName = `${Date.now()}_${file.name.replace(/[^A-Za-z0-9._-]/g, '_')}`;
  const objectRef = ref(storage, `business_media/${currentUser.uid}/gallery/${safeName}`);
  await uploadBytes(objectRef, file, {contentType: file.type});
  const url = await getDownloadURL(objectRef);
  const textarea = document.getElementById('galleryUrls');
  textarea.value = [textarea.value.trim(), url].filter(Boolean).join('\n');
...
async function renderPublicProfile(slug) {
  ...
  applyQrAjnPublicProfileSeo(p, slug);
  root.innerHTML = shell(`<main class="public-wrap">${profileCard(p)}</main>`);
  bindNavigation();
  bindProfileActions(p);
  trackBusinessEvent(p.slug || normalizeSlug(slug), 'profile_view');
}
...
function bindProfileActions(p) {
  document.querySelectorAll('[data-action]').forEach((link) => {
    link.addEventListener('click', () => {
      trackBusinessEvent(p.slug, link.dataset.action || 'unknown');
    });
  });
  document.getElementById('saveContactBtn')?.addEventListener('click', () => {
    trackBusinessEvent(p.slug, 'contact_save');
    const vcard = [
...
async function renderDynamicManager() {
  ...
  root.innerHTML = shell(`
    <div class="dashboard">
      <aside class="panel">
        <h1>Dynamic QR</h1>
        <p class="muted">Keep one printed /r/<code> QR while safely editing the destination.</p>
        <div class="notice">Core business publishing is enabled. Ads and paid plans remain disabled until production acceptance testing is complete.</div>
...
function dynamicLinkCard(link) {
  return `<article class="list-item dynamic-card">
    <div><strong>${escapeHtml(link.title || 'Dynamic QR')}</strong><small>${escapeHtml(`${publicDomain}/r/${link.code}`)}</small><p>${escapeHtml(link.destination || '')}</p><span class="status">${link.active === false ? 'PAUSED' : 'ACTIVE'} • ${Number(link.scanCount || 0)} scans</span></div>
  ...
  const open = (link = {}) => {
    ...
    document.getElementById('dynamicForm').onsubmit = async (event) => {
      ...
      try {
        ...
        if (!destination) throw new Error('Enter a valid HTTPS destination.');
...
async function renderAnalytics() {
  if (!await requireAccount()) return;
  root.innerHTML = shell(loading('Loading live analytics…'));
  bindNavigation();

  const ref = doc(db, 'business_analytics', currentUser.uid);
  const paint = (data = {}) => {
    const metrics = [
      ['Dynamic QR scans', data.dynamicScans], ['Profile views', data.profileViews],
      ...
      ['Website clicks', data.websiteClicks], ['Map clicks', data.mapClicks],
      ['Brochure downloads', data.brochureDownloads], ['UPI clicks', data.upiClicks],
      ['Review clicks', data.reviewClicks], ['Appointment clicks', data.appointmentClicks],
      ['Lead submissions', data.leads],
    ];
    root.innerHTML = shell(`
      <section class="section">
        <div class="section-head"><div><h1>Live business analytics</h1><p>Privacy-safe counters are written by the trusted backend and streamed from Firestore.</p></div></div>
        ...
        <div class="panel section"><h2>Trusted writes only</h2><p class="muted">Public clients cannot directly modify analytics counters. Dynamic redirects use <code>/r/&lt;code&gt;</code>; public profiles use <code>/b/&lt;slug&gt;</code>. Raw QR payloads and raw IP addresses are not stored in business analytics.</p></div>
      </section>
    `);
    bindNavigation();
  };

  const unsubscribe = onSnapshot(ref, (snapshot) => paint(snapshot.data() || {}), (error) => renderError(error.message));
  const stop = () => { unsubscribe(); window.removeEventListener('popstate', stop); };
  window.addEventListener('popstate', stop, {once: true});
}


async function trackBusinessEvent(slug, action) {
  const normalized = normalizeSlug(slug || '');
  if (!normalized || !action || !runtimeConfig.eventEndpoint) return;
  try {
    await fetch(runtimeConfig.eventEndpoint, {
      method: 'POST',
      headers: {'content-type': 'application/json'},
      keepalive: true,
      body: JSON.stringify({slug: normalized, action}),
    });
  } catch {
    // Engagement analytics is best-effort and must not interrupt profile actions.
  }
}
  ...
  root.innerHTML = shell(`
    <section class="section">
      <div class="panel">
        ...
        <a class="btn primary" href="/builder" data-nav>Create business profile</a>
      </div>
    </section>
  `);
  bindNavigation();
...
function renderFaq() {
  const faqs = [
    ['Does a customer need the QR AJN app?', 'No. Public business cards open in a normal browser.'],
    ['Can I update a card after printing the QR?', 'Yes. Keep the same public profile or dynamic QR URL and update the destination or profile content later.'],
    ['Is login required to scan QR codes?', 'No. Login is only required for cloud business features.'],
    ['Are premium plans or ads active?', 'No. QR AJN 7.1 intentionally ships with Play Billing and advertising disabled while the core business workflows are stabilized.'],
    ['Can I use my own domain?', 'Yes. The included Vercel setup connects qrajn.online or another domain you own.'],
  ];
  root.innerHTML = shell(`
    <section class="section">
      <div class="section-head"><div><h1>Frequently asked questions</h1><p>Simple answers for QR AJN users and businesses.</p></div></div>
      <div class="list">${faqs.map(([q,a]) => `<article class="card"><h3>${escapeHtml(q)}</h3><p class="muted">${escapeHtml(a)}</p></article>`).join('')}</div>
    </section>
  `);
  bindNavigation();
}
...
async function renderAdmin() {
  ...
  if (!adminSnapshot.exists() || adminSnapshot.data()?.active !== true) {
    return renderError('This account is not an active QR AJN administrator.');
  }
  const [profiles, users] = await Promise.all([
    getDocs(query(collection(db, 'business_profiles'), limit(50))),
    getDocs(query(collection(db, 'users'), limit(50))),
  ]);
  root.innerHTML = shell(`
    <section class="section">
      <div class="section-head"><div><h1>Admin overview</h1><p>Read-only production overview. Destructive account actions remain server-side.</p></div></div>
      <div class="metrics">
        <article class="card metric"><strong>${profiles.size}</strong><span>Profiles loaded</span></article>
        <article class="card metric"><strong>${users.size}</strong><span>Users loaded</span></article>
      </div>
      <div class="panel section"><h2>Security model</h2><p class="muted">Create an <code>admins/{uid}</code> Firestore document with <code>active: true</code> from a trusted console or backend. Client code cannot grant itself admin access.</p></div>
    </section>
  `);
  bindNavigation();
}
...
function renderError(message) {
  setQrAjnSeo({
    title: 'QR AJN â€” Page unavailable',
    description: 'The requested QR AJN page is unavailable, private or could not be found.',
    canonical: `${publicDomain}${location.pathname}`,
...
function optionalHttpUrl(value) {
  ...
  if (!safe) throw new Error(`Invalid HTTPS URL: ${text}`);
  return safe;
}
...
function safeActionUrl(value) {
  try {
    const url = new URL(String(value || '').trim());
    return ['https:', 'tel:', 'mailto:', 'upi:'].includes(url.protocol)
      ? url.href
      : '';
  } catch {
    return '';
