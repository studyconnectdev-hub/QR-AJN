...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/health') return safeJson(res, 200, {ok:true, service:'qr-ajn-backend', version:'5.0.0'});
    ...
    if (req.method === 'GET' && redirectMatch) {
      ...
      const destination = selectDestination(result, req.headers['user-agent']);
      const validated = await validatePublicUrl(destination);
      queueScanEvent(result, req, validated.url.toString()).catch(() => {});
    ...
    if (req.method === 'POST' && url.pathname === '/v1/links') {
      ...
      const maximumScans = optionalPositiveInteger(body.maximumScans ?? body.maxScans);
      const result = await createDynamicLink({
        code: body.code,
        ownerUid: identity.uid,
        destination: validated.url.toString(),
        androidDestination: await optionalPublicUrl(body.androidDestination),
        iosDestination: await optionalPublicUrl(body.iosDestination),
        desktopDestination: await optionalPublicUrl(body.desktopDestination),
        fallbackDestination: await optionalPublicUrl(body.fallbackDestination),
        title: String(body.title || body.label || '').slice(0, 120),
        expiresAt,
        maximumScans,
...
function selectDestination(link, userAgentValue) {
  const userAgent = String(userAgentValue || '').toLowerCase();
  if (userAgent.includes('android') && link.androidDestination) {
    return link.androidDestination;
  }
  if (/(iphone|ipad|ipod)/.test(userAgent) && link.iosDestination) {
    return link.iosDestination;
  }
  if (!/(android|iphone|ipad|ipod|mobile)/.test(userAgent) && link.desktopDestination) {
    return link.desktopDestination;
  }
  return link.destination || link.fallbackDestination;
}

async function optionalPublicUrl(value) {
  if (value == null || String(value).trim() === '') return '';
  const validated = await validatePublicUrl(value);
  return validated.url.toString();
}

async function queueScanEvent(link, req, destination) {
  const ip = String(req.headers['x-forwarded-for'] || '').split(',')[0].trim();
  const dailySalt = new Date().toISOString().slice(0, 10);
  const ipHash = ip
    ? crypto.createHash('sha256').update(`${dailySalt}:${ip}`).digest('hex')
    : null;
  const userAgent = String(req.headers['user-agent'] || '').slice(0, 300);
  const event = {
    linkId: link.id,
    ownerUid: link.ownerUid || null,
  ...
  await db.collection('scan_events').add(event);

  if (link.ownerUid) {
    await db.collection('business_analytics').doc(String(link.ownerUid)).set({
      dynamicScans: admin.firestore.FieldValue.increment(1),
